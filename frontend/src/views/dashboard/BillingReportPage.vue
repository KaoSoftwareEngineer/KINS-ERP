    ...
    <table class="rp-table" style="min-width:700px">
      ...
      <tbody>
        ...
        <tr v-for="(row, idx) in filtered" :key="row.id" :class="{ 'is-sel': selRow === row }" @click="selRow = row">
          ...
          <td class="rp-c">{{ fmtDate(row.bill_date) }}</td>
...
export default {
  ...
  inject: ['dash'],
  data() { return { rows: [], selRow: null, filter: { date: '', customer: '', q: '' }, sort: { key: 'br_no', dir: 'desc' } }; },
  computed: {
    filtered() {
      let list = this.rows.slice(); const f = this.filter;
      if (f.date) list = list.filter(r => this.fmtDate(r.bill_date) === this.fmtDate(f.date));
      if (f.customer) list = list.filter(r => (r.customer || '').toLowerCase().includes(f.customer.trim().toLowerCase()));
      if (f.q) { const q = f.q.trim().toLowerCase(); list = list.filter(r => (r.br_no || '').toLowerCase().includes(q) || (r.customer || '').toLowerCase().includes(q)); }
    ...
    selTotal() { return this.selItems.reduce((s, it) => s + (Number(it.total) || 0), 0); },
    sumTotal() { return this.filtered.reduce((s, r) => s + (Number(r.total_amount) || 0), 0); },
  },
  mounted() { this.load(); },
  methods: {
    async load() {
      try {
        const res = await fetch('/api/customer-billings', { headers: { Authorization: 'Bearer ' + this.dash.token } });
        if (res.status === 401) { this.dash.sessionExpired && this.dash.sessionExpired(); return; }
        const d = await res.json();
        this.rows = d.billings || d.rows || []; this.selRow = null;
      } catch (e) { this.rows = []; }
    },
    ...
    reset() { this.filter = { date: '', customer: '', q: '' }; this.selRow = null; },
    exportExcel() {
      const head = ['เลขที่วางบิล', 'วันที่', 'วันที่ครบกำหนด', 'ลูกค้า', 'ยอดรวม'];
      const lines = [head.join('\t')];
      this.filtered.forEach(r => lines.push([r.br_no, this.fmtDate(r.bill_date), this.fmtDate(r.due_date), r.customer, r.total_amount].join('\t')));
      const blob = new Blob(['﻿' + lines.join('\n')], { type: 'text/csv;charset=utf-8;' });
      const a = document.createElement('a'); a.href = URL.createObjectURL(blob); a.download = 'รายงานวางบิลลูกค้า.csv'; a.click();
    },
  },
};
</script>
...
.rp-muted { color: var(--muted); } .rp-mono { font-family: 'Courier New', monospace; } .rp-loss { color: #c0392b; }
