...
<div class="fr-page-compact">
  ...
  <div class="section" style="margin-top: 12px;">
    ...
    <div class="fr-filter-grid grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-[18px] gap-y-[14px]">
      <div class="fr-field-group">
        ...
        <input type="text" v-model="fabricStore.fiFilters.search" :placeholder="dash.t[dash.lang].searchFabricPlaceholder" />
      </div>
      <div class="fr-field-group">
      ...
      <div class="fr-field-group">
        ...
        <div class="fr-sku-range">
          <input type="text" v-model="fabricStore.fiFilters.skuFrom" :placeholder="dash.t[dash.lang].fromWord" />
          <span>—</span>
          <input type="text" v-model="fabricStore.fiFilters.skuTo" :placeholder="dash.t[dash.lang].toWord" />
        </div>
      </div>
      <div class="fr-field-group">
      ...
      <div class="fr-field-group fr-checkbox-group">
        ...
        <label for="fiSubstitute">{{ dash.t[dash.lang].substituteLabel }}</label>
      </div>
    </div>
  ...
  <div class="fr-summary fr-summary-row">
    ...
    <div class="fr-summary-actions">
      <button v-if="fabricStore.fiSelected.length > 0" class="btn-small" style="color: var(--danger); border-color: var(--danger);" @click="fabricStore.fiBulkDelete">
        🗑️ {{ dash.t[dash.lang].delete }} ({{ fabricStore.fiSelected.length }})
      </button>
  ...
  <div class="section fr-table-section" style="margin-top: 8px; padding: 0; overflow: hidden;">
      ...
      <table class="fr-table">
        <thead>
          <tr>
            ...
            <th class="fr-th-sort" @click="fabricStore.fiSort('type')">{{ dash.t[dash.lang].typeLabel }} <span class="fr-sort-icon">{{ fabricStore.fiSortBy === 'type' ? (fabricStore.fiSortDir === 'asc' ? '▲' : '▼') : '' }}</span></th>
            <th class="fr-th-sort" @click="fabricStore.fiSort('sku')">{{ dash.t[dash.lang].skuLabel }} <span class="fr-sort-icon">{{ fabricStore.fiSortBy === 'sku' ? (fabricStore.fiSortDir === 'asc' ? '▲' : '▼') : '' }}</span></th>
            <th class="fr-th-sort" @click="fabricStore.fiSort('colors')">{{ dash.lang === 'th' ? 'จำนวนสี' : 'Colors' }} <span class="fr-sort-icon">{{ fabricStore.fiSortBy === 'colors' ? (fabricStore.fiSortDir === 'asc' ? '▲' : '▼') : '' }}</span></th>
            <th class="fr-th-sort" @click="fabricStore.fiSort('name')">{{ dash.t[dash.lang].nameLabel }} <span class="fr-sort-icon">{{ fabricStore.fiSortBy === 'name' ? (fabricStore.fiSortDir === 'asc' ? '▲' : '▼') : '' }}</span></th>
            <th>{{ dash.t[dash.lang].structureLabel }}</th>
            <th>{{ dash.t[dash.lang].compositionLabel }}</th>
            <th>{{ dash.t[dash.lang].widthLabel }}</th>
            <th>Finishing</th>
            <th>{{ dash.t[dash.lang].weightLabel }}</th>
        ...
        <tbody>
          <tr v-for="(item, idx) in fabricStore.fiPagedItems" :key="item.sku">
            ...
            <td>
              <button class="fr-color-badge" @click="dash.fiOpenShades(item)" :title="dash.t[dash.lang].manageShades">
            ...
            <td>
              <div class="fr-action-group">
                <button class="fr-action-btn edit" :title="dash.t[dash.lang].edit" @click="fabricStore.fiEditItem(item)">
                ...
                <button class="fr-action-btn delete" :title="dash.t[dash.lang].delete" @click="fabricStore.fiDeleteItem(item)">
                ...
                <button class="fr-action-btn view" :title="dash.t[dash.lang].viewDetails" @click="fabricStore.fiViewItem(item)">
    ...
    <div class="xl-pagination" v-if="fabricStore.fiFilteredItems.length > 0">
      ...
      <button class="fr-btn-util" :disabled="fabricStore.fiPage === 1" @click="fabricStore.fiPrevPage">{{ dash.t[dash.lang].prevPage }}</button>
      <span><strong>{{ dash.t[dash.lang].pageWord }} {{ fabricStore.fiPage }} / {{ fabricStore.fiTotalPages }}</strong> — {{ dash.t[dash.lang].showingWord }} {{ (fabricStore.fiPage - 1) * fabricStore.fiPageSize + 1 }}-{{ Math.min(fabricStore.fiPage * fabricStore.fiPageSize, fabricStore.fiFilteredItems.length) }} {{ dash.t[dash.lang].ofTotalWord }} {{ fabricStore.fiFilteredItems.length }} {{ dash.t[dash.lang].itemsUnit }}</span>
      <button class="fr-btn-util" :disabled="fabricStore.fiPage === fabricStore.fiTotalPages" @click="fabricStore.fiNextPage">{{ dash.t[dash.lang].nextPage }}</button>
    </div>
  </div>
  ...
  <div class="erp-overlay" v-if="fabricStore.fiShowModal" @click.self="fabricStore.fiCloseModal">
    <div class="erp-modal">
      <div class="erp-modal-head">
        ...
        <button class="erp-x" @click="fabricStore.fiCloseModal" :title="dash.t[dash.lang].close">✕</button>
      </div>
      ...
      <fieldset :disabled="fabricStore.fiModalMode === 'view'" style="border:0;margin:0;padding:0;min-width:0;">
        ...
        <div class="erp-grid">
          <div class="erp-field"><label>{{ dash.t[dash.lang].typeLabel }} <span class="erp-req">*</span></label>
            <select v-model="fabricStore.fiNewItem.type"><option value="">{{ dash.t[dash.lang].selectTypeOpt }}</option><option v-for="opt in dash.fiTypeOptions" :key="opt" :value="opt">{{ opt }}</option></select>
          </div>
          <div class="erp-field"><label>{{ dash.t[dash.lang].skuLabel }} <span class="erp-req">*</span></label><input type="text" v-model="fabricStore.fiNewItem.sku" placeholder="e.g. FI-SAT01" /></div>
          <div class="erp-field erp-col-2"><label>{{ dash.t[dash.lang].nameLabel }}</label><input type="text" v-model="fabricStore.fiNewItem.name" :placeholder="dash.t[dash.lang].fabricNamePlaceholder" /></div>
          <div class="erp-field"><label>{{ dash.t[dash.lang].structureLabel }}</label>
            <select v-model="fabricStore.fiNewItem.structure"><option value="">{{ dash.t[dash.lang].selectStructureOpt }}</option><option v-for="opt in dash.fiStructureOptions" :key="opt" :value="opt">{{ opt }}</option></select>
          </div>
          <div class="erp-field"><label>{{ dash.t[dash.lang].compositionLabel }}</label>
            <select v-model="fabricStore.fiNewItem.composition"><option value="">{{ dash.t[dash.lang].selectCompositionOpt }}</option><option v-for="opt in dash.fiCompositionOptions" :key="opt" :value="opt">{{ opt }}</option></select>
          </div>
          <div class="erp-field"><label>{{ dash.t[dash.lang].widthLabel }} <span class="erp-req">*</span></label>
            <select v-model="fabricStore.fiNewItem.width"><option value="">{{ dash.t[dash.lang].selectWidthOpt }}</option><option v-for="opt in dash.fiWidthOptions" :key="opt" :value="opt">{{ opt }}</option></select>
          </div>
          <div class="erp-field"><label>Finishing</label>
            <select v-model="fabricStore.fiNewItem.finishing"><option value="">{{ dash.lang === 'th' ? 'เลือก Finishing' : 'Select Finishing' }}</option><option v-for="opt in dash.fiFinishingOptions" :key="opt" :value="opt">{{ opt }}</option></select>
          </div>
          <div class="erp-field"><label>{{ dash.t[dash.lang].weightLabel }}</label>
            <select v-model="fabricStore.fiNewItem.weight"><option value="">{{ dash.t[dash.lang].selectWeightOpt }}</option><option v-for="opt in dash.fiWeightOptions" :key="opt" :value="opt">{{ opt }} GSM</option></select>
          </div>
        ...
        <div class="erp-grid">
          <div class="erp-field erp-col-2"><label>{{ dash.t[dash.lang].descriptionLabel }}</label><input type="text" v-model="fabricStore.fiNewItem.description" :placeholder="dash.t[dash.lang].moreDescPlaceholder" /></div>
          <div class="erp-field"><label>{{ dash.t[dash.lang].productionDaysLabel }}</label><input type="number" min="0" v-model="fabricStore.fiNewItem.productionDays" placeholder="0" /></div>
          ...
          <div class="erp-field"><label>{{ dash.t[dash.lang].substituteLabel }}</label>
            <select v-model="fabricStore.fiNewItem.substitute"><option value="no">{{ dash.t[dash.lang].noWord }}</option><option value="yes">{{ dash.t[dash.lang].yesWord }}</option></select>
          </div>
          <div class="erp-field"></div>
          <div class="erp-field erp-col-2"><label>{{ dash.t[dash.lang].imageLabel }}</label>
            <div class="fr-file-input">
              <span class="fr-file-name">{{ fabricStore.fiNewItem.imageName || dash.t[dash.lang].noFileSelected }}</span>
              <button type="button" class="fr-file-btn" @click="$refs.fiFileInput.click()" :title="dash.t[dash.lang].attachImageTitle">
      ...
      <div class="erp-modal-foot">
        <button class="erp-btn erp-btn-cancel" @click="fabricStore.fiCloseModal">{{ fabricStore.fiModalMode === 'view' ? dash.t[dash.lang].close : dash.t[dash.lang].cancelWord }}</button>
