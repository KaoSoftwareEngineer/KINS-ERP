<template>
<div class="po-page">
  <div class="po-titlebar">🎨 {{ dash.t[dash.lang].receiveDyedFabricTitle }}</div>
  ...
  <div class="po-head">
    <div class="po-head-col">
      ...
      <div class="po-field"><label>{{ dash.t[dash.lang].receiptNoLabel }}</label><input :value="form.in_no" readonly class="po-ro" /></div>
    </div>
    <div class="po-head-col">
      <div class="po-field"><label>{{ dash.t[dash.lang].orderNoLabel }}</label>
        <select v-model="form.order_ref" @change="selectOrder">
          <option value="">{{ dash.t[dash.lang].selectDyeOrderOpt }}</option>
          <option v-for="o in dyeOrders" :key="o.id" :value="o.dye_no">{{ o.dye_no }}</option>
        </select>
      </div>
      <div class="po-field"><label>{{ dash.t[dash.lang].factoryLabel }}</label>
        <input list="gr-factories" v-model="form.factory" :placeholder="dash.t[dash.lang].factoryPlaceholder" />
        <datalist id="gr-factories"><option v-for="f in factoryOptions" :key="f" :value="f" /></datalist>
      </div>
    </div>
    <div class="po-head-col">
      <div class="po-field"><label>{{ dash.t[dash.lang].warehouseLabel }}</label>
    ...
    <table class="po-item-table">
      <thead>
        <tr>
          ...
          <th>{{ dash.t[dash.lang].skuLabel }}</th><th>{{ dash.t[dash.lang].colorCodeLabel }}</th><th>{{ dash.t[dash.lang].nameLabel }}</th><th>{{ dash.t[dash.lang].widthLabel }}</th><th style="width:70px;">{{ dash.t[dash.lang].foldLabel }}</th>
          <th style="width:90px;">{{ dash.t[dash.lang].qtyLabel }}</th><th style="width:100px;">{{ dash.t[dash.lang].pricePerUnitLabel }}</th><th style="width:110px;">{{ dash.t[dash.lang].priceLabel }}</th>
      ...
      <tbody>
        <tr v-for="(row, idx) in items" :key="row._key">
          ...
          <td class="po-row-actions">
            <button class="po-ic po-fold" :title="dash.t[dash.lang].foldDetailTitle" @click="openFoldDrawer(idx)">
            ...
            <button class="po-ic po-copy" :title="dash.t[dash.lang].copyRowTitle" @click="copyRow(idx)">
      ...
      <tfoot>
        <tr class="po-foot-row">
          <td colspan="5" class="po-foot-label">{{ dash.t[dash.lang].totalWord }}</td>
  ...
  <div class="po-footer">
    ...
    <div class="po-footer-btns">
      ...
      <template v-else>
        <button class="po-btn po-btn-receipt" @click="openReceiptPdf">🧾 {{ dash.t[dash.lang].receiptPdfBtn }}</button>
        <button class="po-btn po-btn-barcode" @click="printBarcodes">🏷️ {{ dash.t[dash.lang].barcodeBtnWord }}</button>
        <button class="po-btn po-btn-new" @click="resetForm">🔄 {{ dash.t[dash.lang].receiveNewBtn }}</button>
      </template>
    </div>
  </div>
      ...
      <div class="fd-drawer">
        <div class="fd-head">
          <div class="fd-head-txt">
            <span class="fd-head-title">{{ dash.t[dash.lang].foldDetailTitle }}</span>
            <span v-if="foldTitle" class="fd-head-sub">{{ foldTitle }}</span>
          </div>
          <button class="fd-x" :title="dash.t[dash.lang].close" @click="closeFoldDrawer">
            <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round"><path d="M18 6 6 18M6 6l12 12"/></svg>
          </button>
        </div>
        <div class="fd-body" v-if="foldRow">
          <p class="fd-hint">{{ dash.t[dash.lang].foldHint }}</p>
          <div class="fd-list">
            <div class="fd-row fd-row-head">
              <div class="fd-c-per">{{ dash.t[dash.lang].perFoldLabel }}</div>
              <div class="fd-c-cnt">{{ dash.t[dash.lang].foldCountLabel }}</div>
              <div class="fd-c-sum">{{ dash.t[dash.lang].sumYardsLabel }}</div>
            ...
            <div class="fd-row" v-for="(f, i) in foldRow.folds" :key="i">
              ...
              <div class="fd-c-act">
                <button class="fd-mini fd-mini-del" :title="dash.t[dash.lang].removeFoldTitle" @click="removeFold(i)">
          ...
          <button class="fd-add-btn" @click="addFold(foldRow.folds.length - 1)">
            ...
            {{ dash.t[dash.lang].addFoldBtn }}
          </button>

          <div class="fd-summary">
            <div class="fd-sum-item"><span class="fd-sum-lbl">{{ dash.t[dash.lang].totalFoldCountLabel }}</span><span class="fd-sum-val">{{ foldTotalCount }}</span></div>
            <div class="fd-sum-item fd-sum-main"><span class="fd-sum-lbl">{{ dash.t[dash.lang].totalYardsLabel }}</span><span class="fd-sum-val">{{ foldTotalYards.toFixed(2) }}</span></div>
          </div>
        </div>
        <div class="fd-foot-bar">
          <button class="po-btn" @click="closeFoldDrawer">{{ dash.t[dash.lang].cancelWord }}</button>
          <button class="po-btn po-btn-save" @click="applyFolds">{{ dash.t[dash.lang].save }}</button>
        </div>
      </div>
    </div>
  </transition>
...
export default {
  ...
  data() {
    return {
      form: { in_no: '', receipt_date: new Date().toISOString().slice(0, 10), order_ref: '', factory: '', warehouse: 'Warehouse', remark: '' },
      items: [this.newRow()],
      warehouseOptions: ['Warehouse', 'Factory'],
      discountMode: 'none', discountValue: 0, vatMode: 'none',
      fabrics: [], factoryOptions: [], dyeOrders: [],
  ...
  computed: {
    ...
    foldTitle() { const r = this.foldRow; return r ? [r.sku, r.color].filter(Boolean).join(' – ') : ''; },
  ...
  methods: {
    newRow() { return { _key: (this._seq = (this._seq || 0) + 1), sku: '', color: '', name: '', width: '', lot: '', fold: 0, qty: 0, unit_price: null, folds: [{ perFold: null, count: null }], rolls: [], shadeOptions: [] }; },
    ...
    async loadNextNo() {
      try {
        const res = await fetch('/api/dyed-receipts/next-no', { headers: { Authorization: 'Bearer ' + this.dash.token } });
    ...
    async loadRefs() {
      ...
      try {
        const r2 = await fetch('/api/factories', { headers: { Authorization: 'Bearer ' + this.dash.token } });
        const d2 = await r2.json();
        if (d2.ok) this.factoryOptions = (d2.items || []).map(f => f.name).filter(Boolean);
      } catch (e) {}
      try {
        const r3 = await fetch('/api/dye-orders', { headers: { Authorization: 'Bearer ' + this.dash.token } });
        const d3 = await r3.json();
        if (d3.ok) this.dyeOrders = d3.orders || [];
      } catch (e) {}
    },
    // เลือกใบสั่งย้อม → ดึงผ้า + เฉดสีมาเติมตาราง
    selectOrder() {
      const ord = this.dyeOrders.find(o => o.dye_no === this.form.order_ref);
      if (!ord) return;
      if (ord.factory) this.form.factory = ord.factory;
      let product = {}, orderItems = [];
      try { product = JSON.parse(ord.product_json || '{}'); } catch (e) {}
      try { orderItems = JSON.parse(ord.items_json || '[]'); } catch (e) {}
      if (orderItems.length) {
        this.items = orderItems.map(it => {
          const r = this.newRow();
          r.sku = product.sku || '';
          r.name = product.name || '';
          r.width = product.width || '';
          r.color = it.shade || it.color_code || '';
          r.unit_price = it.unit_price != null ? Number(it.unit_price) : null;
          return r;
        });
      }
    },
    ...
    resetForm() {
      this.form = { in_no: '', receipt_date: new Date().toISOString().slice(0, 10), order_ref: '', factory: '', warehouse: 'Warehouse', remark: '' };
    ...
    async save() {
      ...
      try {
        const res = await fetch('/api/dyed-receipts', {
          method: 'POST',
    ...
    async openReceiptPdf() {
      ...
      const html = `
        ...
        <div class="doc">
        ${docBrandHeader('ใบรับผ้าย้อม (Dyed Fabric Receipt)')}
        <div class="meta">
          <div><b>เลขที่ออร์เดอร์ :</b> ${esc(f.order_ref) || '-'}<br><b>โรงงาน :</b> ${esc(f.factory) || '-'}<br><b>คลัง :</b> ${esc(f.warehouse)}</div>
          <div><b>เลขที่รับสินค้า :</b> ${esc(f.in_no)}<br><b>วันที่ :</b> ${esc(f.receipt_date)}</div>
        </div>
        <table>
      ...
      try { await buildDocPdf(html, { filename: 'ใบรับผ้าย้อม-' + (f.in_no || '') + '.pdf' }); }
      catch (e) { this.dash.fbFail('สร้าง PDF ไม่สำเร็จ'); }
    },
  },
};
