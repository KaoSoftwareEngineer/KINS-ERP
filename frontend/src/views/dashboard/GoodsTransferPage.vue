...
export default {
  ...
  data() {
    return {
      form: { tr_no: '', transfer_date: new Date().toISOString().slice(0, 10), from_wh: 'Warehouse', to_wh: 'Cut Piece', remark: '' },
      items: [],
      warehouseOptions: ['Warehouse', 'Cut Piece', 'Factory'],
      fabrics: [], saved: false, savedMsg: '', _seq: 1,
  ...
  methods: {
    async loadNextNo() {
      try {
        const res = await fetch('/api/stock-transfers/next-no', { headers: { Authorization: 'Bearer ' + this.dash.token } });
        const d = await res.json();
        if (d.ok) this.form.tr_no = d.tr_no;
      } catch (e) {}
    },
    async loadFabrics() {
      try {
    ...
    async selectFabric(f) {
      const row = { _key: (this._seq = (this._seq || 0) + 1), sku: f.sku || '', color: '', group: f.type || '', shade: '', width: f.width || '', qty: null, unit: 'หลา', shadeOptions: [] };
      try {
    ...
    onShade(row) {
      const parts = (row.shade || '').split(/\s*-\s*/);
      row.color = parts[0] || '';
    },
    removeRow(idx) { this.items.splice(idx, 1); },
    resetForm() {
      this.form = { tr_no: '', transfer_date: new Date().toISOString().slice(0, 10), from_wh: 'Warehouse', to_wh: 'Cut Piece', remark: '' };
      this.items = []; this.saved = false; this.savedMsg = '';
      this.loadNextNo();
    },
    async save() {
      if (this.items.length === 0) { this.dash.fbFail('กรุณาเลือกสินค้าที่จะย้ายอย่างน้อย 1 รายการ'); return; }
      if (this.form.from_wh === this.form.to_wh) { this.dash.fbFail('คลังต้นทางและปลายทางต้องต่างกัน'); return; }
      this.dash.fbLoading('กำลังบันทึก...');
      const payload = {
        ...this.form,
        items: this.items.map(r => ({ sku: r.sku, color: r.color, group: r.group, shade: r.shade, width: r.width, qty: r.qty, unit: r.unit })),
      };
      try {
    ...
    openPdf() {
      ...
      win.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>${esc(f.tr_no)}</title>
...
.po-items { padding: 12px 16px; min-height: 120px; }
