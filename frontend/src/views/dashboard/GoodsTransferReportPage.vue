...
<div class="rp-page">
  ...
  <div class="rp-filter">
    ...
    <div class="rp-f"><label>จากคลัง</label>
      <select v-model="filter.fromWh" @change="load"><option value="">ทั้งหมด</option><option v-for="o in fromOptions" :key="o" :value="o">{{ o }}</option></select>
    </div>
    ...
    <table class="rp-table">
      <thead>
        <tr>
          ...
          <th class="rp-sortable" @click="toggleSort('tr_no')">เลขที่ย้ายสินค้า <span class="rp-sort" :class="{ on: sort.key === 'tr_no' }">{{ sortIcon('tr_no') }}</span></th>
          ...
          <th class="rp-sortable" @click="toggleSort('from_wh')">จากคลัง <span class="rp-sort" :class="{ on: sort.key === 'from_wh' }">{{ sortIcon('from_wh') }}</span></th>
      ...
      <tbody>
        <tr v-for="(row, idx) in sortedItems" :key="row.id" :class="{ 'is-sel': selRow === row }" @click="selectRow(row)">
          ...
          <td class="rp-c">
            <button class="rp-ic rp-print" title="พิมพ์ใบย้าย" @click.stop="printTransfer(row)">
  ...
  <div class="rp-table-wrap rp-rolls">
    <table class="rp-table">
      ...
      <tbody>
        ...
        <tr v-if="selItems.length === 0"><td colspan="10" class="rp-empty">{{ selRow === null ? 'คลิกแถวด้านบนเพื่อดูรายการที่ย้าย' : 'ไม่มีรายการในใบย้ายนี้' }}</td></tr>
      </tbody>
