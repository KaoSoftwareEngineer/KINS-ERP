...
<div class="rp-page">
  <div class="rp-titlebar">
    <span>🔀 {{ dash.t[dash.lang].goodsTransferReportTitle }}</span>
  ...
  <div class="rp-filter">
    ...
    <div class="rp-f"><label>{{ dash.t[dash.lang].fromWarehouseLabel }}</label>
      <select v-model="filter.fromWh" @change="load"><option value="">{{ dash.t[dash.lang].allWord }}</option><option v-for="o in fromOptions" :key="o" :value="o">{{ o }}</option></select>
    </div>
    ...
    <div class="rp-f"><label>{{ dash.t[dash.lang].colorCodeLabel }}</label><input v-model="filter.color" @keyup.enter="load" /></div>
    ...
    <table class="rp-table">
      <thead>
        <tr>
          ...
          <th class="rp-sortable" @click="toggleSort('tr_no')">{{ dash.t[dash.lang].transferNoLabel }} <span class="rp-sort" :class="{ on: sort.key === 'tr_no' }">{{ sortIcon('tr_no') }}</span></th>
          ...
          <th class="rp-sortable" @click="toggleSort('from_wh')">{{ dash.t[dash.lang].fromWarehouseLabel }} <span class="rp-sort" :class="{ on: sort.key === 'from_wh' }">{{ sortIcon('from_wh') }}</span></th>
      ...
      <tbody>
        <tr v-for="(row, idx) in sortedItems" :key="row.id" :class="{ 'is-sel': selRow === row }" @click="selectRow(row)">
          ...
          <td class="rp-c">
            <button class="rp-ic rp-print" :title="dash.t[dash.lang].printTransferTitle" @click.stop="printTransfer(row)">
        ...
        <tr v-if="items.length === 0"><td colspan="8" class="rp-empty">{{ dash.t[dash.lang].noDataFoundMsg }}</td></tr>
      </tbody>
      <tfoot v-if="items.length">
        <tr>
          <td colspan="5" class="rp-r rp-bold">{{ dash.t[dash.lang].totalWord }}</td>
    ...
    <table class="rp-table">
      <thead>
        <tr>
          ...
          <th class="rp-r">{{ dash.t[dash.lang].qtyLabel }}</th><th>{{ dash.t[dash.lang].unitLabel }}</th><th>{{ dash.t[dash.lang].barcodeLabel }}</th><th style="width:44px;"></th>
        </tr>
      </thead>
      <tbody>
        ...
        <tr v-if="selItems.length === 0"><td colspan="10" class="rp-empty">{{ selRow === null ? dash.t[dash.lang].clickRowToViewTransferMsg : dash.t[dash.lang].noItemsInTransferMsg }}</td></tr>
      </tbody>
      <tfoot v-if="selItems.length">
        <tr><td colspan="6" class="rp-r rp-bold">{{ dash.t[dash.lang].totalWord }}</td><td class="rp-r rp-bold">{{ fmt(selQty) }}</td><td colspan="3"></td></tr>
      </tfoot>
    </table>
  </div>
</div>
...
export default {
  ...
  data() {
    return {
      filter: { q: '', fromWh: '', toWh: '', sku: '', color: '' },
  ...
  computed: {
    fromOptions() { return [...new Set(this.items.map(i => i.from_wh).filter(Boolean))].sort(); },
  ...
  methods: {
    ...
    async load() {
      ...
      try {
        const res = await fetch('/api/reports/stock-transfers' + (qs ? '?' + qs : ''), { headers: this.authHeaders() });
    ...
    reset() { this.filter = { q: '', fromWh: '', toWh: '', sku: '', color: '' }; this.load(); },
    toggleSort(key) {
    ...
    async exportExcel() {
      ...
      const body = this.sortedItems.map(r => [r.tr_no, this.fmtDate(r.transfer_date), r.from_wh, r.to_wh, Number(r.folds) || 0, Number(r.qty) || 0]);
      const XLSX = await import('xlsx');
      const sheet = XLSX.utils.aoa_to_sheet([head, ...body]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, sheet, 'ย้ายสินค้า');
      XLSX.writeFile(wb, `รายงานการย้ายสินค้า-${new Date().toISOString().slice(0, 10)}.xlsx`);
    },
  },
};
</script>
