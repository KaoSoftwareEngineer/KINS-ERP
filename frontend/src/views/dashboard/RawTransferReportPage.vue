...
export default {
  name: 'RawTransferReportPage',
  inject: ['dash'],
  data() {
    return {
      filter: { q: '', toWh: '', sku: '' },
      items: [], summary: { lots: 0, qty: 0 },
  ...
  methods: {
    ...
    async load() {
      ...
      try {
        const res = await fetch('/api/reports/raw-transfers' + (qs ? '?' + qs : ''), { headers: this.authHeaders() });
        if (res.status === 401) { this.dash.sessionExpired(); return; }
        const d = await res.json();
        if (d.ok) {
          this.items = d.items || [];
          this.summary = { lots: 0, qty: 0, ...(d.summary || {}) };
          this.selRow = null; this.selItems = [];
        }
      } catch (e) { this.dash.fbFail('โหลดรายงานไม่สำเร็จ'); }
    },
    reset() { this.filter = { q: '', toWh: '', sku: '' }; this.load(); },
    toggleSort(key) {
    ...
    async printTransfer(row) {
      ...
      const rowsHtml = (row.items || []).map((it, i) => `<tr><td>${i + 1}</td><td>${esc(it.sku)}</td><td>${esc(it.name)}</td><td>${esc(it.width)}</td><td>${esc(it.lot)}</td><td style="text-align:right">${this.fmt(it.qty)}</td><td>${esc(it.unit)}</td><td>${esc(it.warehouse)}</td></tr>`).join('');
      const html = `
        <style>
        ...
        <h2>ใบย้ายผ้าดิบ ${esc(row.tg_no)}</h2>
        <div class="meta">วันที่: ${esc(this.fmtDate(row.transfer_date))} · ไปยังคลัง: ${esc(row.to_wh)}</div>
        <table><thead><tr><th>ที่</th><th>รหัสสินค้า</th><th>ชื่อ</th><th>หน้ากว้าง</th><th>เลขที่ล็อต</th><th>จำนวน</th><th>หน่วย</th><th>จากคลัง</th></tr></thead>
        <tbody>${rowsHtml || '<tr><td colspan="8" style="text-align:center">ไม่มีรายการ</td></tr>'}</tbody>
        <tfoot><tr><th colspan="5" style="text-align:right">รวม</th><th style="text-align:right">${this.fmt(row.qty)}</th><th colspan="2"></th></tr></tfoot></table>`;
      try { await buildDocPdf(html, { filename: 'ใบย้ายผ้าดิบ-' + row.tg_no + '.pdf', format: 'a4' }); }
      catch (e) { this.dash.fbFail('สร้าง PDF ไม่สำเร็จ'); }
    },
    async exportExcel() {
      const head = ['เลขที่ย้ายสินค้า', 'วันที่', 'ไปยังคลัง', 'จำนวนล็อต', 'จำนวนรวม'];
      const body = this.sortedItems.map(r => [r.tg_no, this.fmtDate(r.transfer_date), r.to_wh, Number(r.lots) || 0, Number(r.qty) || 0]);
      const XLSX = await import('xlsx');
      const sheet = XLSX.utils.aoa_to_sheet([head, ...body]);
      const wb = XLSX.utils.book_new();
      XLSX.utils.book_append_sheet(wb, sheet, 'ย้ายผ้าดิบ');
      XLSX.writeFile(wb, `รายงานการย้ายผ้าดิบ-${new Date().toISOString().slice(0, 10)}.xlsx`);
    },
  },
};
</script>
