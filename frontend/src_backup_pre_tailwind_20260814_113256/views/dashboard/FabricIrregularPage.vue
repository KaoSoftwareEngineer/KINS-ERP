...
<div class="fr-page-compact">
  <div class="header">
    ...
    <div class="header-actions">
      <button class="btn-small btn-primary" @click="dash.fiOpenAdd">+ {{ dash.t[dash.lang].add }} {{ dash.pageTitle('fabric-irregular') }}</button>
    </div>
  </div>
  ...
  <div class="section" style="margin-top: 12px;">
    ...
    <div class="fr-filter-actions">
      <button class="fr-btn-util" @click="dash.fiSearch">
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.35-4.35"/></svg>
        {{ dash.t[dash.lang].searchWord }}
      </button>
      <button class="fr-btn-util" @click="dash.fiResetFilters">
  ...
  <div class="fr-summary">{{ dash.t[dash.lang].foundItems }} {{ dash.fiFilteredItems.length }} {{ dash.t[dash.lang].itemsUnit }}</div>

  <div class="section" style="margin-top: 8px; padding: 0; overflow: hidden;">
    <div class="fr-table-scroll">
      <table class="fr-table">
        ...
        <tbody>
          <tr v-for="(item, idx) in dash.fiFilteredItems" :key="item.sku">
            <td>{{ idx + 1 }}</td>
            <td>{{ item.type }}</td>
          ...
          <tr v-if="dash.fiLoading" class="fr-empty-row">
            <td colspan="13">กำลังโหลดข้อมูล...</td>
          </tr>
